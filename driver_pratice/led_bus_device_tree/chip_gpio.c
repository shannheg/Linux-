...
static int chip_gpio_probe(struct platform_device *pdev)
{
    ...
    //const struct board_led_platform_data *pdata = dev_get_platdata(&pdev->dev);原先使用platform_data获取数据，现在改为设备树中获取

    leds = of_get_child_by_name(pdev->dev.of_node, "leds");//获取名字为"leds"的子节点，此处为dp_led下的leds节点
    if(!leds){
        printk("leds node is not exist\n");
        return -EINVAL;
    }
    for_each_child_of_node(leds, child){
        if(index >= LED_MAX){
            printk("exceed LED_MAX\n");
            of_node_put(leds);
            return -EINVAL;
        }
        err = of_property_read_u32(child, "group", &group);
        if (err) {
            printk("Failed to read group property: %d\n", err);
            of_node_put(leds);
            return err;
        }
        err = of_property_read_u32(child, "pin", &pin);
        if (err) {
            printk("Failed to read pin property: %d\n", err);
            of_node_put(leds);
            return err;
        }

        g_ledpins[index] = (group << 16) | pin;//设置寄存器是需要低16位中的值置1，还需将高16位的使能置1
        g_ledcnt++;
